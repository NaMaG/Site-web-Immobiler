...
if (!function_exists('ere_get_format_number')) {
    ...
    {
	    $number_digit = 10000;
	    $number = intval($number * $number_digit);
	    if ($number) {
		    $number_floor = $number/$number_digit;
		    $number_decimal = $number % $number_digit;
		    $dec_point = ere_get_option('decimal_separator', '.');
		    $thousands_sep = ere_get_option('thousand_separator', ',');
		    $number_floor =  number_format($number_floor, $decimals, $dec_point, $thousands_sep);
		    if ($number_decimal > 0) {
			    $number_decimal = substr(($number_decimal/$number_digit) . '',2) ;
			    return $number_floor . $dec_point . $number_decimal;
		    }
		    return $number_floor;
	    } else {
		    return 0;
